import React from 'react';
import {getVideoImage} from './vimeo';
import Radium from 'radium';

const ShowcaseThumb = Radium(({videos, name}) => (<div style={{
    position: 'relative',
    textAlign: 'center',
    color: 'white',
    transition: 'font-size 0.5s ease, filter 0.5s ease',
    fontSize: '20px',
    filter: 'grayscale(80%)',
    ':hover': {
        fontSize: '24px',
        filter: 'none'
    }
}}>
    <img style={{height: '30vh'}} key={`sc-thumb-${name}`} src={getVideoImage(videos[0])} />
    <h3 style={{position: 'absolute', bottom: '0', left: '50%', transform: 'translate(-50%, -50%)'}}>{name}</h3>
</div>));

const Porfolio = ({showcases}) => {
    const gridSize = Math.ceil(Math.sqrt(Object.keys(showcases).length));
    console.log(`repeat(1fr, ${gridSize}fr)`)
    return (<div style={{
        display: 'flex',
        justifyContent: 'center',
        justifyItems: 'center',
        flexWrap: 'wrap',
    }}>
        {Object.keys(showcases).map(name => <ShowcaseThumb key={`sc-thumb-${name}`} name={name} videos={showcases[name]} />)}
    </div>);
};

Porfolio.propTypes = {
    showcases: Object
};

export default Porfolio;